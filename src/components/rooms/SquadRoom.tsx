...
export const SquadRoom = () => {
  const { squads, profile, createSquad, leaveSquad, setCurrentSquad, currentSquadId } = useGameStore();
  ...
  const handleCreateSquad = () => {
    ...
    const squad = createSquad(newSquadName);
  ...
  const handleJoinSquad = () => {
    ...
    toast.error('Squad not found. Please check the code.');
    setJoinCode('');
  };
        ...
        {squads.length === 0 ? (
          <PixelPanel className="text-center py-12">
            ...
            <p className="font-game text-xl text-muted-foreground mb-6">
              Create a squad or join one with a code to start your journey together!
            </p>
            ...
            {squads.map((squad) => (
              ...
              >
                <PixelPanel
                  ...
                  onClick={() => setCurrentSquad(squad.id)}
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex -space-x-2">
                      {squad.members.map((member) => (
                        <PixelAvatar key={member.id} seed={member.avatarSeed} size="sm" />
                      ))}
                    </div>
                    <span className="font-game text-lg text-muted-foreground">
                      {squad.members.length}/12
                    </span>
                  </div>
                </PixelPanel>
              </motion.div>
              ...
              <div className="space-y-3">
                {currentSquad.members
                  ...
                  .map((member, index) => (
                    <div
                      key={member.id}
                      className="flex items-center gap-4 bg-card/20 p-3 pixel-border"
                    >
                      <span className="font-pixel text-lg text-game-gold w-8">
                        #{index + 1}
                      </span>
                      <PixelAvatar seed={member.avatarSeed} size="sm" />
                      ...
                      <div className="text-right">
                        <p className="font-game text-xl text-game-gold">{member.points} pts</p>
            ...
            >
              <PixelPanel variant="dialog" className="w-full max-w-md">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <PixelButton onClick={handleCreateSquad} className="flex-1">
                      Create
                    </PixelButton>
                    <PixelButton
                      variant="secondary"
            ...
            >
              <PixelPanel variant="dialog" className="w-full max-w-md">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <PixelButton onClick={handleJoinSquad} className="flex-1">
                      Join
                    </PixelButton>
                    <PixelButton
                      variant="secondary"
